...
export function WorkStatusProvider({ children }) {
  ...
  // Auto reset after 2 hours of shift end
  useEffect(() => {
  ...
  useEffect(() => {
    const loadWorkStatus = async () => {
      try {
        ...
        const logsJson = await AsyncStorage.getItem(
          `attendo_work_logs_${today}`
        );
  ...
  const updateWorkStatus = async (action, timestamp) => {
    try {
      ...
      // Trigger refresh for WeeklyStatus
      ...
      if (
        action === "check_out" ||
        action === "complete" ||
        (action === "go_work" && currentShift?.onlyGoWorkMode)
      ) {
