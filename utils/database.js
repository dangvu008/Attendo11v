...
export const initializeDatabase = async () => {
  try {
    ...
    if (!initialized) {
      // Create a transaction-like operation to ensure all or nothing
      const operations = [
        AsyncStorage.setItem(SHIFTS_KEY, JSON.stringify([])),
        AsyncStorage.setItem(WORK_LOGS_KEY, JSON.stringify([])),
        AsyncStorage.setItem(WORK_STATUS_KEY, JSON.stringify({})),
        AsyncStorage.setItem(NOTES_KEY, JSON.stringify([])),
