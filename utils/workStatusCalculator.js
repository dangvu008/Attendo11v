...
export const calculateWorkStatus = (logs, shift) => {
  ...
  const parseShiftTime = (timeStr) => {
    if (!timeStr) {
      console.warn("Invalid time string provided to parseShiftTime:", timeStr);
  ...
  if (!checkInLog || !checkOutLog) {
    return {
      ...
      checkInTime: checkInLog
        ? format(parseISO(checkInLog.timestamp), "HH:mm:ss")
        : null,
      checkOutTime: checkOutLog
        ? format(parseISO(checkOutLog.timestamp), "HH:mm:ss")
        : null,
      totalWorkTime: 0,
      overtime: 0,
  ...
  if (checkInTime > startTime) {
    lateMinutes =
      Math.ceil(differenceInMinutes(checkInTime, startTime) / 30) * 30;
  }
  ...
  if (checkOutTime < officeEndTime) {
    earlyMinutes =
      Math.ceil(differenceInMinutes(officeEndTime, checkOutTime) / 30) * 30;
  }
